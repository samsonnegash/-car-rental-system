...
class LoginPage extends RunnerPage
{
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("carsusers")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$sqlHead = "select * from ".$this->connection->addTableWrappers( "carsusers" ) ." where ";
		$sql = array();
		if( $userIdField ) {
			$sql []= $sqlHead
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "carsusers", $data, $blobfields, $this );
			if( !$retval )
				return false;
